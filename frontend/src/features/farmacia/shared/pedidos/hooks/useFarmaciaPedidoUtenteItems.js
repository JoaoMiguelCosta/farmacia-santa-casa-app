// src/features/farmacia/shared/pedidos/hooks/useFarmaciaPedidoUtenteItems.js
...
function compareItems(firstItem, secondItem) {
  ...
  return String(firstItem?.medicamento || "").localeCompare(
    String(secondItem?.medicamento || ""),
    "pt-PT",
    {
      sensitivity: "base",
    },
...
export function useFarmaciaPedidoUtenteItems({ items = [], pageSize = 10 }) {
  ...
  const goToPreviousPage = useCallback(() => {
    setCurrentPage((currentValue) =>
      Math.max(1, Math.min(currentValue, totalPages) - 1),
    );
  ...
  const goToNextPage = useCallback(() => {
    setCurrentPage((currentValue) =>
      Math.min(totalPages, Math.min(currentValue, totalPages) + 1),
    );
